...
class posSession(models.Model):
    ...
    def sage_sopro_pos_report(self):
        ...
        if file_path:
            ...
            # Vérification de la présence de config_id et de code_pdv_sage
            if self.config_id and self.config_id.code_pdv_sage:
                file = file_path + '/' + str(self.config_id.code_pdv_sage) + '/' + filename
            else:
                # Si le code_pdv_sage n'est pas défini, utilisez un chemin alternatif ou une gestion d'erreur
                logging.error("Code PDV Sage non trouvé pour la configuration de la session POS : %s", self.name)
                return
